(function(imageproc) {
    ...
    imageproc.comicColor = function(inputData, outputData, saturation) {
        ...
        for (var i = 0; i < inputData.data.length; i += 4) {
            ...
            hsl.s = hsl.s * saturation > 1 ? 1 : hsl.s * saturation;
            ...
            var mini = 0;
            var mind = Math.hypot(r - palette[0][0], g - palette[0][1], b - palette[0][2]);
            for(var j = 1; j < palette.length; j++){
              var t1 = Math.hypot(r - palette[j][0], g - palette[j][1], b - palette[j][2]);
              if(t1 < mind){
                mini = j;
                mind = t1;
              }
